...
class CustomFlutterNativeTextField extends StatefulWidget {
  ...
  final void Function(double contentHeight)? internalArgs;

  const CustomFlutterNativeTextField({
    super.key,
    this.inputKey,
    this.placeholder,
    this.screenHeight,
  ...
  @override
  State<CustomFlutterNativeTextField> createState() => _CustomFlutterNativeTextFieldState();
}

class _CustomFlutterNativeTextFieldState extends State<CustomFlutterNativeTextField> {
  bool showSuffixIcon = false;
  @override
  void initState() {
    super.initState();

    widget.controller?.addListener(refreshSuffixIconState);
    widget.focusNode?.addListener(refreshSuffixIconState);

    // Initialize default text if no text is set in the provided controller
    if (widget.controller != null && widget.defaultText.isNotEmpty && widget.controller!.text.isEmpty) {
  ...
  @override
  void dispose() {
    widget.controller?.removeListener(refreshSuffixIconState);
    widget.focusNode?.removeListener(refreshSuffixIconState);
    super.dispose();
  }
  ...
  void refreshSuffixIconState() {
    if (widget.alwaysShowSuffixIcon) {
      ...
      showSuffixIcon = widget.suffixIcon != null && (widget.focusNode?.hasFocus ?? false) && (widget.controller?.text.isNotEmpty ?? false);
    }
    setState(() {});
  }
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Align(
                  ...
                  child: NativeTextInput(
                    ...
                    internalArgs: (contentHeight) => widget.internalArgs!(contentHeight),
              ...
              if (showSuffixIcon && widget.suffixIcon != null)
                Align(
...
// From Flutter Native Text Input -------
  ...
  void _inputValueChanged(String? text, int? lineIndex) async {
    if (text != null) {
      ...
      if (mounted && value != null && value != _contentHeight) {
        _contentHeight = value;
        setState(() {});
        log("Set contentHeight");
        widget.internalArgs!(_contentHeight);
      }
    }
  }
