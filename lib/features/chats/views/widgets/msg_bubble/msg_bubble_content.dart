...
class MsgBubbleContent extends StatefulWidget {
  ...
  @override
  State<MsgBubbleContent> createState() => _MsgBubbleContentState();
}

class _MsgBubbleContentState extends State<MsgBubbleContent> {
  @override
  Widget build(BuildContext context) {
    final bool hasTaggedMessage = widget.messageModel.taggedMessageID != null && widget.messageModel.taggedMessageID!.isNotEmpty;
    final bool hasAttachment =
        widget.messageModel.mediaUrl != null && widget.messageModel.mediaUrl!.isNotEmpty && MessageTypeExtension.fromInt(widget.messageModel.mediaType) != MessageType.text;
    final bool hasMedia = widget.messageModel.mediaUrl != null && widget.messageModel.mediaUrl!.isNotEmpty;
    final bool hasMediaCaption = widget.messageModel.mediaCaption != null && widget.messageModel.mediaCaption!.isNotEmpty;
    final bool isJustImgOverlay = hasMedia &&
        MessageTypeExtension.fromInt(widget.messageModel.mediaType) != MessageType.text &&
        (widget.messageModel.mediaCaption == null || (widget.messageModel.mediaCaption != null && widget.messageModel.mediaCaption!.isEmpty));
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (hasTaggedMessage) IntrinsicWidth(child: BuildTaggedMsgWidget(messageModel: widget.messageModel, taggedMsgColor: widget.taggedMsgColor,)),
        if (hasAttachment) BuildAttachmentWidget(messageModel: widget.messageModel, isSender: widget.isSender, isJustImgOverlay: isJustImgOverlay),
        if (widget.messageModel.content.isNotEmpty || hasMediaCaption)
          Padding(
            ...
            child: TimestampedChatMessage(
                ...
                sentAt: Padding(
                  ...
                  child: BuildTimeStampWidget(
                      date: DateFormat.jm().format(widget.messageModel.sentAt),
                      isSender: widget.isSender,
                      deliveredAt: widget.messageModel.deliveredAt,
                      seenAt: widget.messageModel.seenAt),
                )),
          )
      ],
    );
...
class BuildTimeStampWidget extends StatelessWidget {
  ...
  const BuildTimeStampWidget({super.key, required this.date, required this.isSender, this.seenAt, this.deliveredAt});
